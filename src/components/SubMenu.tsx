...
function SubMenu({ data, type }: { data: any, type: string }) {
  ...
  const handleNavigate = (e: MouseEvent<HTMLButtonElement>, item: any) => {
    e.stopPropagation()
        ...
        <ul className="relative grid grid-cols-3 rounded-xl p-3 text-sm text-white z-30 overflow-hidden">
          ...
          {data.map((item:any, index:number) => (
            <li key={index} className="h-10 hover:opacity-70">
              <button type='button' name='button' onClick={(e) => handleNavigate(e, item)} className="w-full h-full flex items-center whitespace-nowrap cursor-pointer">
                {item.name}
              </button>
            </li>
          ))}
