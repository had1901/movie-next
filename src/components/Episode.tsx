...
function Episode({ data }:{ data: any }) {
  ...
  return (
    <div id='episodes'>
        {data.item.episodes.length > 0 && data.item.episodes.map((ep: any, index: number) => (
            <div key={index}>
                ...
                {currentIndex === index + 1  && <ul className='grid grid-cols-10 mt-8'>
                    {ep.server_data.length > 0 && ep.server_data.map((film: any, index: number) => (
                        <li 
                            key={index} 
                            className={`${epsIndex === index + 1 ? 'text-(--text-main-color) bg-(--bg-main-color)' : 'text-white' } flex items-center  gap-1 justify-center font-medium mr-2 bg-(--bg-label) py-2 px-4 text-center rounded-md cursor-pointer`}
