...
function MovieCard({ item, imgDomain, index, ratio = 'aspect-[2/3]', clippath = false, isBackdrop=false, isSearch=false }:
  ...
  return (
    <Link ref={isSearch ? null : cardRef} href={`/detail-movie/${item.slug}`} title={item.name} className='inline-block w-full cursor-pointer' >
        <div 
          className={`relative overflow-hidden  hover:opacity-80 transition ${ratio}`} 
          style={{ clipPath: `${clippath ? clip : ''}`}}>
            <Image 
                src={`${imgDomain}/uploads/movies/${isBackdrop ? item.poster_url : item.thumb_url}`} 
                sizes="100%"
                fill
                alt="thumbnail"
