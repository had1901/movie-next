...
function MovieCard({ item, imgDomain='https://img.ophim.live', index, ratio = 'aspect-[2/3]', clippath = false, isBackdrop=false, isSearch=false }:
  { item: any, imgDomain: string, index: number, ratio?: string, clippath?: boolean, isBackdrop?: boolean, isSearch?: boolean }) {
  ...
  const clip = `${index % 2 !== 0 && clippath 
    ...
    const handleClick = (slug: string) => {
      ...
      let arrParse: string[] = itemSlug ? JSON.parse(itemSlug) : []

      // Loại bỏ slug nếu đã có
      arrParse = arrParse.filter(s => s !== slug)

      // Thêm slug mới lên đầu
      arrParse.unshift(slug)

      localStorage.setItem('_films', JSON.stringify(arrParse))
    }
  return (
    <Link onClick={() => handleClick(item.slug)} ref={isSearch ? null : cardRef} href={`/detail-movie/${item.slug}`} title={item.name} className='inline-block w-full cursor-pointer' >
        <div 
          className={`relative overflow-hidden  hover:opacity-80 transition ${ratio}`} 
          style={{ clipPath: `${clippath ? clip : ''}`}}>
            <Image 
                src={imgDomain && (item.poster_url || item.thumb_url)
                  ? `${imgDomain}/uploads/movies/${isBackdrop ? item.poster_url : item.thumb_url}`
                  : "/fallback.png"
                } 
                sizes="100%"
                fill
                alt="thumbnail"
                className='select-none pointer-events-none object-cover rounded-xl shadow-2xl '
            />
        ...
        <h2 className='text-white font-medium text-center pt-2 px-2 line-clamp-1 hover:text-(--text-main-yellow)'>{item.name}</h2>
